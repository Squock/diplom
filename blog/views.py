...
def download(request):
    ...
    for u in userdoc:
        if(request.user.id == u.user_id):
            print(u.user_id)
            for t in test:
                docfiles = open('C:\\myproject\\Новая папка\\django 1.8\\mysite\\media\\uploads\\' + str(request.user.id) + t.title + '_сtest2.docx', 'rb')
...
#        user_id = models.ForeignKey(User)
        ...
        if(find == True):
            print(f.id)
            idd = f.id
            b = Filling(id=test, author_id=request.user, inn=inputInn, ogrn=inputOgrn, name=inputName, city=inputCity, address=inputAddress)
            c = orgdata.objects.create(doc_id=f.id, user_id=request.user.id)
            #c = orgdata(id=org_id, doc_id=f.id, user_id=request.user.id)
            c.save()
            b.save()
        else:
            b = Filling.objects.create(author_id=request.user, inn=inputInn, ogrn=inputOgrn, name=inputName, city=inputCity, address=inputAddress)
            c = orgdata.objects.create(doc_id=f.id, user_id=request.user.id)
            c.save()
        ...
        #p.text = inputWname
                ...
                for i in range(len(inline)):
                        ...
                        if Dictionary['orgname'] in inline[i].text:
                            text = inline[i].text.replace(Dictionary['orgname'], inputName)
                            inline[i].text = text
        document.save('C:\\myproject\\document\\' + str(request.user.id) + t.title + '_сtest2.docx')"""
        ...
        #p.text = inputWname
            ...
            for i in range(len(inline)):
                    ...
                    if Dictionary['position'] in inline[i].text:
                        ...
                        inline[i].text = text
            document.save('C:\\myproject\\document\\' + str(request.user.id) + '_сtest2.docx')
